...
REPLICATION = {'class' : 'SimpleStrategy', 'replication_factor' : 3} if os.getenv("ENV") == "development" else {'class' : 'NetworkTopologyStrategy', 'dc1' : 3, 'dc2' : 2}   
AUTH_PROVIDER = None # set to PlainTextAuthProvider(USERNAME, PASSWORD) after you configure cassandra auth
os.environ['CQLENG_ALLOW_SCHEMA_MANAGEMENT'] = '1'

class init:
	def __new__(cls, *args, **kwargs):
		try:
			cls.__cluster = Cluster(contact_points=HOSTS, port=PORT, load_balancing_policy=DCAwareRoundRobinPolicy(local_dc="datacenter1"), 
									auth_provider=AUTH_PROVIDER, protocol_version=4) # db hosts and port to start clusters
