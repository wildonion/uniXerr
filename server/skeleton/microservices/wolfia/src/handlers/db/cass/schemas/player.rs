...
impl Chat{
    ...
    pub async fn init(session: Arc<CassSession>){
        let create_player_data_table = "CREATE TABLE IF NOT EXISTS wolfia.user_chat (id UUID, user_id int, friend_id int, message text, chattime timestamp, servertime timestamp, PRIMARY KEY((id, user_id, friend_id), chattime, servertime));";
    ...
    pub async fn save(&self, session: Arc<CassSession>){
        let insert_player_data_cql = "INSERT INTO wolfia.user_chat (id, user_id, friend_id, message, chattime, servertime) VALUES (?, ?, ?, ?, ?, ?, toUnixTimestamp(now()))";
