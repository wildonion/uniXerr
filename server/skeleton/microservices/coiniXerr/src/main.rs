...
async fn main() -> std::io::Result<()>{
    ...
    tokio::spawn(async move { //-- tokio::spawn() is an asynchronous multithreaded and event loop based task spawner and scheduler which takes a task of type future of a process and shares it between multiple threads using its job queue channel protocol, so every type in the task must be Send + Sync + 'static + Future and cloneable
        let mut i = 0_usize;
        let heavy_number = 2;
