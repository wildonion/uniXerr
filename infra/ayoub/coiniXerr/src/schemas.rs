...
//
//// by default, events generated by the remaining members are delegated to 
//// NetworkBehaviourEventProcess implementations those must be provided by 
//// the user on the type that NetworkBehaviour is derived on
//// alternatively, we can specify #[behaviour(event_process = false)] in this case, 
//// users should provide a custom out_event and implement From for each of the 
//// event types generated by the struct members.
//
//// From trait has been implemented for each event variant
//// which will return that variant as the current event.
#[allow(clippy::large_enum_variant)] //// this will allow large size differences between enum variants
pub enum P2PAppBehaviourEvent{ //// this will be used inside the loop to change the code flow of the started swarm based on these events
...
impl From<GossipsubEvent> for P2PAppBehaviourEvent {
    fn from(event: GossipsubEvent) -> Self {
        P2PAppBehaviourEvent::Gossipsub(event) //// returning the gossipsub event
    }
}

impl From<mdns::Event> for P2PAppBehaviourEvent{
    fn from(event: mdns::Event) -> Self{
        P2PAppBehaviourEvent::Mdns(event) //// returning the mdns event
    }
} 

impl From<String> for P2PAppBehaviour{
    fn from(event: String) -> Self{
        P2PAppBehaviourEvent::Input(event) //// returning the input event
    }
}

impl From<P2PChainResponse> for P2PAppBehaviour{
    fn from(event: P2PChainResponse) -> Self{
        P2PAppBehaviourEvent::LocalChainResponse(event) //// returning the local chain response event
    }
}
