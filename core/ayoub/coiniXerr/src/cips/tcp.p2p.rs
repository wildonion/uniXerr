...
pub async fn bootstrap(storage: Option<Arc<Storage>>, env_vars: HashMap<String, String>){
    ...
    while let Some((transaction, validator, coiniXerr_actor_system)) = mempool_receiver.recv().await{ //-- waiting for each transaction to become available to the down side of channel (receiver) for mining process cause sending is done asynchronously 
        ...
        while std::mem::size_of_val(&current_block) <= env_vars.get("MAX_BLOCK_SIZE").unwrap().parse::<usize>().unwrap(){ //-- returns the dynamically-known size of the pointed-to value in bytes by passing a reference or pointer to the value to this method - push incoming transaction into the current_block until the current block size is smaller than the env_vars.get("MAX_BLOCK_SIZE")
            ...
            if std::mem::size_of_val(&current_block) > env_vars.get("MAX_BLOCK_SIZE").unwrap().parse::<usize>().unwrap(){
