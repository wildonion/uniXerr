...
// NOTE - `Reserve` struct contains some data structures to store on chain infos about tokens and their owners at runtime
...
#[near_bindgen] //// implementing the #[near_bindgen] proc macro attribute on `Reserve` struct to compile all its methods to wasm so we can call them in near cli
impl Reserve{
