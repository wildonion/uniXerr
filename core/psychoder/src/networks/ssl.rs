...
NOTE - feature vectoers learning and representation or extracting semantic attributes using dimensionality reduction algorithms like GA, TSNE, PCA and VAE 
        ...
        in verification tasks and the category that the new input must be in, in classification tasks using transformers in CV and NLP. 
...
➔ in ssl or meta learning we're looking for finding the semantic meaning between input features and put them into a latent space or an embedding layer of extracted features or attributes to campare this layer with the features layer of unseen and contrastive inputs 
